function [ci,bootstat,bcaparam]  = bootci(nboot,bootfun,varargin)
...
%   length of 'Streams' must equal the number of workers used by BOOTCI.
%   If a parallel pool is already open, this will be the size of the
%   parallel pool.  If a parallel pool is not already open, then MATLAB may
%   try to open a pool for you (depending on your installation and preferences).
%   To ensure more predictable results, it is best to use the PARPOOL
%   command and explicitly create a parallel pool prior to invoking BOOTCI
%   with 'UseParallel' set to TRUE.
%   
...
%   RANDSAMPLE, PARPOOL, PARFOR, GCP, PARALLELSTATS.
...
switch (lower(type))
    ...
    case 'bca'
        [ci,bootstat,bcaparam] = bootbca(obsstat,nboot,fun,alpha,weights,bootstrpOptions,data{:});
    case {'stud','student'}
...
%-------------------------------------------------------------------------
function [ci,bstat,bcaparam] = bootbca(stat,nboot,bootfun,alpha,weights,bootstrpOptions,varargin)
