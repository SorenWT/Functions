...
if isempty(a) || isempty(b)
    warning('No data left after removing NaNs and Infs')
   return 
end
...
if CheckInput(varargin,'Cov')
   ...
   warning('Too few points to calculate p value')
   corrrho = corr(a,b,'rows','pairwise','Type',type);
   corrp = 0;
end
...
h = scatter(a,b,scattersize,scatterclr,'filled');
...
if isnan(corrrho) && isnan(corrp) && exist('tb','var')
    delete(tb)
end
