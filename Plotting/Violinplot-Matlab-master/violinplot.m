...
if CheckInput(varargin,'xvals')
    xvals = EasyParse(varargin,'xvals');
    varargin = removeargs(varargin,{'xvals'});
end
...
if isa(data, 'dataset') || isstruct(data) || istable(data)
    ...
    set(gca, 'XTick', xvals, 'XTickLabels', catnames);
    set(gca,'Box','on');
    return
...
elseif isnumeric(data) % numeric input
    ...
    if hascategories && numel(data) == numel(cats)
        ...
        set(gca, 'XTick', xvals, 'XTickLabels', catnames_labels);
        set(gca,'Box','on');
        return
    else
        data = {data};
