...
for i = 1:size(allcovars,2)
    if length(unique(allcovars.(n{i}))) > 0.25*length(allcovars.(n{i}))
...
for i = 1:length(mdlcovars)
    ...
    if length(unique(tmp)) < 0.25*length(tmp)
...
tblcommand.grp = zeros(0); tblcommand.InputFile = zeros(0);
...
for i = 1:size(allisc,1) 
    for ii = 1:(i-1)
        if i ~= ii
            ...
            WriteBrik (repmat(allisc(i,ii),size(Vfunc,1),size(Vfunc,2),size(Vfunc,3)), InfoDelOut, OptDelOut);
...
gltcode = struct;
gltcode.ave = zeros(nfacts,1); gltcode.ave(1) = 1;
for i = 1:size(tblfacts,2)
