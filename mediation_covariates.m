function [out] = mediation_covariates(Y,X,M,C,options)
...
% modified by SWT on 04/04/2019
if isfield(options,'display_mod') && options.display_mod
    out.hf = figure('color',[1 1 1],'name','mediation analysis');
    
    indvar_name = options.indvar;
    depvar_name = options.depvar;
    med_name = options.mediator;
    
    ha = axes('parent',out.hf,'nextplot','add','position',[0.1300    0.1100    0.7750    0.5],'units','normalized');
    hc(1) = rectangle('parent',ha,'position',[-2 0 1 1],'curvature',[1 1]);
    ht(1) = text(-1.5,0.5,indvar_name,'parent',ha,'HorizontalAlignment','Center','FontSize',16);
    hc(2) = rectangle('parent',ha,'position',[4 0 1 1],'curvature',[1 1]);
    ht(2) = text(4.5,0.5,depvar_name,'parent',ha,'HorizontalAlignment','Center','FontSize',16);
    hc(3) = rectangle('parent',ha,'position',[1 -2 1 1],'curvature',[1 1]);
    ht(3) = text(1.5,-1.5,med_name,'parent',ha,'HorizontalAlignment','Center','FontSize',16);
    
    indvar_name = strrep(indvar_name,newline,' ');
    depvar_name = strrep(depvar_name,newline,' ');
    med_name = strrep(med_name,newline,' ');
    ...
    hq(1) = quiver(-1,0.5,5.5,0,'parent',ha,'color',col);
    ht(4) = text(1.5,0.65,['R^2=',num2str(round(1e3*de)/10),'% (p=',num2str(out.p(1,2)),')'],'parent',ha,'HorizontalAlignment','Center');
    ht(4).FontSize = 12;
    % effect of X on M
    if p2 > alpha
        col = 'r';
    ...
    hq(2) = quiver(0.5*cos(pi/6)-1.5,0.5-0.5*sin(pi/6),2+0.5*cos(pi/6),sin(pi/6)-2,'parent',ha,'color',col);
    ht(5) = text(0,-0.5,['R^2=',num2str(round(1e3*a2.R2_a)/10),'% (p=',num2str(out.p(2,1)),')'],'parent',ha,'HorizontalAlignment','Center');
    ht(5).FontSize = 12;
    % effect of M on Y (in the presence of X)
    if p4 > alpha
        col = 'r';
