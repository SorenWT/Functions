function [r,p,ci,n] = partialcorrswt(x,y,z,varargin)
...
x = x.*nanmask(~isinf(x)); y = y.*nanmask(~isinf(y)); z = z.*nanmask(~isinf(z));
...
if EasyParse(argsin,'removeoutliers',true)
    x = x.*nanmask(~isoutlier(x)); y = y.*nanmask(~isoutlier(y)); z = z.*nanmask(~isoutlier(z));
