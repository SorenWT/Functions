function [pos,pnts,whichview] = openpose_readposture(filein,extended,manualview)
...
if length(datin.people) > 1 && max(personconfs) < 0.5
...
if isfield(datin,'whichview')
    ...
    if exist(imgfile,'file') && manualview
        I = imread(imgfile);
        imshow(I)
        set(gcf,'units','normalized')
        figpos = get(gcf,'position')
