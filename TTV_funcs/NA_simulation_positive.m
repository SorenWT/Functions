    ...
    for n = nvals
        ...
        parfor c = 1:48
            ...
            for cc = 1:75 % 5-min resting state
                ...
                spont = NormOntoRange(spont,[1 2])*subAmpl;
...
for c = 1:length(mvals)
    ...
    parfor cc = 1:length(nvals)
        ...
        med{c,cc} = mediation_covariates(vert(squeeze(mean(all_datacalc{c,cc}.raw.ersp(1,poststim_real,:),2))),...
            all_restmeas{c,cc}.rel_bp(:,1),vert(squeeze(mean(all_datacalc{c,cc}.raw.ersp(1,prestim_real,:),2))),...
...
save('simulation_positive_alloutputs.mat','allstats_pt','allstats_ttv','all_datacalc','all_restmeas','med','-v7.3')
