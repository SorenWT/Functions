...
settings = struct;
settings.pseudo.prestim = 401:450; 
settings.pseudo.poststim= 501:900; 
settings.real.prestim = 901:950; 
settings.real.poststim = 1001:1400;
...
%             rest{c} = ft_freqsimulation_swt(cfg);
	    ...
	    cfg.osci.s1.freq = 3;
            ...
            for cc = 1:128
                ...
                evo{i}(cc,:) = [zeros(1,1000) corrfact{i}(cc)*sin((1:150)*pi/150) zeros(1,850)]
            end
...
%                        timefreq_data{c+1}.parent = 2;
 ...
 %           end
%            freqdata = [];
            timefreq_data = cell(1,1);            
            tmpcfg = []; tmpcfg.bpfilter = 'yes'; tmpcfg.bpfreq = [2 4]; tmpcfg.hilbert = 'complex';
            timefreq_data{1} = ft_preprocessing(tmpcfg,task{i});
            
            newsettings = struct;
            newsettings.units = 'prcchange';
            newsettings.tfparams.fbands = {'Delta'};
            newsettings.tfparams.method = 'hilbert';
...
prestim_pseudo = 401:450; poststim_pseudo = 501:900; prestim_real = 901:950; poststim_real = 1001:1400;
