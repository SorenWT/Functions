...
if any(any(isnan(X))) || any(any(isnan(ystart)))
   ...
   % Remove NaN rows, avoid converting row vector to column vector
   [~,~,X,ystart] = statslib.internal.removenanrows(X,ystart);
   if isempty(X)
...
if strcmpi(algorithm,'exact')
    ...
    if N==1
        ...
        if ~strcmpi(distance,'precomputed')
...
[Y,loss] = tsneEmbedding(ystart,probMatX,exaggeration,learnrate,...
    numprint,verbose,options,algorithm,theta,colidx,rowcnt,cumrowcnt);
end % tsne
...
N = size(Y,1);
numeratorProbMatY = 1 ./ (1 + internal.stats.pdist2mex(Y',Y','sqe',[],[],[],true));
numeratorProbMatY(1:N+1:end) = 0;
probMatY = max(numeratorProbMatY./sum(numeratorProbMatY(:)),realmin(class(Y)));
numeratorProbMatY = numeratorProbMatY.*(probMatX - probMatY);
grad = 4 * (diag(sum(numeratorProbMatY,1))-numeratorProbMatY) * Y; 
end

function [Y,loss] = tsneEmbedding(Y,probMatX,exaggeration,...
              learnrate,numprint,verbose,options,algorithm,theta,colidx,rowcnt,cumrowcnt)
...
while (iter<options.MaxIter && ~stop)
    ...
    if strcmpi(algorithm,'exact')
        ...
        if isempty(colidx) || isempty(rowcnt) || all(rowcnt == 0) || isempty(probMatX)
            % Empty joint probability matrix
            grad = zeros(size(Y),'like',Y);
            loss = cast(0,'like',Y);
        else
            [attrForce,repForce,Z] = internal.stats.tsnebhmex(theta,Y',ycenter,ywidth,colidx,rowcnt,probMatX,cumrowcnt);
            grad = 4*(attrForce-repForce)';
            if ( rem(iter,numprint) == 0 )
                loss = internal.stats.tsnelossmex(Y',colidx,rowcnt,probMatX,Z);
            end
        end
    end
...
if haveOutputFcn && ~stop
    optimValues = struct('iteration',iter,'fval',loss,'grad',grad,'Y',Y,...
                         'Exaggeration',exaggeration);
    callOutputFcns(OutputFcn,optimValues,'done');
end

if nargout>1
    % Compute the loss of the final step
    if iter==0
        loss = 0; % did not take any step
    elseif strcmpi(algorithm,'exact')
        entropyX = probMatX(:)'*log(probMatX(:));
        entropyY = probMatX(:)'*log(probMatY(:));
        loss = entropyX - entropyY;
    else
...
function [colidx, rowcnt, cumrowcnt, probvec] = probMatXknn(probMatX,knnidx)
...
rowcnt = sum(P ~= 0,2)';
cumrowcnt = [0,cumsum(full(rowcnt))];
end
