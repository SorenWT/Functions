...
runLME <- function(myData, ModelForm, DM, gltM, intercept, nF, nS, tag) {
   ...
   if(!all(myData == 0)) {     
      ...
      #fm <- summary(lmer(ModelForm, data=DM))
...
# swt edit for fast usage when only doing one model
if(file.exists('isc.csv')){
  require(matlab)
  iscin <- read.csv('isc.csv')
  inData <- repmat(array(iscin$ISC),1,dimx,dimy,dimz)
  inData <- aperm(inData,c(2,3,4,1))
}else{
...
if(!is.na(lop$maskFN)) {
   idx <- which(abs(lop$maskData-1)<0.0001, arr.ind = T)
...
#swt edit for debugging
#require(RJSONIO)

#injson <- toJSON(inData)
#write(injson,file='/Users/Soren/testalldata.json')

#lopjson <- toJSON(lop)
#write(lopjson,file='/Users/Soren/Desktop/Data/jiawei_culture/lop.json')
...
while(is.null(fm)) {
   ...
   #swt edit
   #dmjson <- toJSON(DM)
   #write(dmjson,file='/Users/Soren/Desktop/Data/jiawei_culture/testtbl.json')
   #write.table(DM,file='/Users/Soren/Desktop/Data/jiawei_culture/testtbl.csv')
   
   fm <- lmer(lop$model, data=DM)
